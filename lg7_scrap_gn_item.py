...
if offset is None:
    offset = 0
ready_select_sql = f"select seq_no, source_url from gn_scrap_ready where source_type = '{source_type}' and status = '0' order by seq_no"
    ...
    while True:
        ...
        if res is not None and res.__len__() > 0:
            for record in res:
                ...
                main_page.goto(ready_source_url, wait_until="domcontentloaded")
                full_contents = main_page.content()

                url_tag = soup.select_one('body > main > article > div.topic-table > div.topic > div.topictitle.link > a')

                if url_tag:
                    news_url = url_tag['href']  # 또는 url_tag.get('href')
                    if not news_url.startswith('http'):
                        news_url = f'https://news.hada.io{news_url}'

                # insert
                bm_insert_sql = 'insert into gn_master (news_title, news_desc, news_pub_date, news_url, news_update, full_contents, news_comments) values (?,?,?,?,now(),?,?)'
                cur.execute(bm_insert_sql, (news_title, news_desc, news_pub_date, news_url, full_contents, news_comments))
                conn.commit()
